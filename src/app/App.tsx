import { useState, useRef } from "react";
import { motion, AnimatePresence } from "motion/react";
import {
  Map, Navigation, Users, Siren, Shield,
  Bell, Search, Mic, ChevronUp, ChevronDown,
  MapPin, GitCompare, Layers, X, BarChart3, User,
  Settings, AlertTriangle,
} from "lucide-react";
import { MapCanvas } from "./components/MapCanvas";
import { RoutesPanel } from "./components/RoutesPanel";
import { CommunityPanel } from "./components/CommunityPanel";
import { ContactsPanel } from "./components/ContactsPanel";
import { SOSModal } from "./components/SOSModal";
import { SafetyIntelligence } from "./components/SafetyIntelligence";
import { EmergencyHub } from "./components/EmergencyHub";
import { ProfileHub } from "./components/ProfileHub";
import { RouteComparison } from "./components/RouteComparison";

type Tab = "map" | "safety" | "community" | "emergency" | "profile";

const TABS = [
  { id: "map" as Tab, label: "Map", icon: Map },
  { id: "safety" as Tab, label: "Safety", icon: Shield },
  { id: "community" as Tab, label: "Reports", icon: Users },
  { id: "emergency" as Tab, label: "Emergency", icon: Siren },
  { id: "profile" as Tab, label: "Profile", icon: User },
];

const AREA_SCORE = 82;

function SafetyRing({ score }: { score: number }) {
  const r = 22;
  const circ = 2 * Math.PI * r;
  const offset = circ * (1 - score / 100);
  const color = score >= 80 ? "#10b981" : score >= 60 ? "#f59e0b" : "#f43f5e";
  return (
    <div className="relative w-14 h-14 flex items-center justify-center">
      <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 56 56">
        <circle cx="28" cy="28" r={r} fill="none" stroke="rgba(0,0,0,0.08)" strokeWidth="4" />
        <circle cx="28" cy="28" r={r} fill="none" stroke={color} strokeWidth="4" strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={offset}
          style={{ transition: "stroke-dashoffset 1s ease" }}
        />
      </svg>
      <div className="flex flex-col items-center">
        <span className="text-xs font-bold" style={{ color }}>{score}</span>
        <Shield size={10} style={{ color }} />
      </div>
    </div>
  );
}

function AlertBanner({ onDismiss }: { onDismiss: () => void }) {
  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
      className="mx-4 mt-2 rounded-2xl px-4 py-3 flex items-center gap-3"
      style={{ background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.25)" }}
    >
      <AlertTriangle size={15} className="text-accent shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-foreground text-xs font-medium">Alert: Harassment reported nearby</p>
        <p className="text-muted-foreground text-[11px]">Oak St & 5th Ave · 32 min ago</p>
      </div>
      <button onClick={onDismiss}><X size={14} className="text-muted-foreground" /></button>
    </motion.div>
  );
}

type MapMode = "normal" | "heatmap" | "safespot";

function HeatmapCanvas({ mode }: { mode: MapMode }) {
  // Lightweight overlay rendered via inline SVG for heatmap modes
  const colors = mode === "heatmap"
    ? [
        { cx: "62%", cy: "30%", r: "12%", from: "rgba(244,63,94,0.5)", to: "rgba(244,63,94,0)" },
        { cx: "18%", cy: "70%", r: "9%", from: "rgba(244,63,94,0.4)", to: "rgba(244,63,94,0)" },
        { cx: "50%", cy: "55%", r: "7%", from: "rgba(245,158,11,0.4)", to: "rgba(245,158,11,0)" },
        { cx: "35%", cy: "20%", r: "14%", from: "rgba(16,185,129,0.35)", to: "rgba(16,185,129,0)" },
        { cx: "75%", cy: "65%", r: "10%", from: "rgba(245,158,11,0.35)", to: "rgba(245,158,11,0)" },
      ]
    : [
        { cx: "38%", cy: "55%", r: "8%", from: "rgba(16,185,129,0.5)", to: "rgba(16,185,129,0)" },
        { cx: "52%", cy: "92%", r: "6%", from: "rgba(16,185,129,0.4)", to: "rgba(16,185,129,0)" },
        { cx: "20%", cy: "35%", r: "7%", from: "rgba(99,102,241,0.4)", to: "rgba(99,102,241,0)" },
      ];

  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
      <defs>
        {colors.map((c, i) => (
          <radialGradient key={i} id={`hg${i}`} cx={c.cx} cy={c.cy} r={c.r} gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor={c.from} />
            <stop offset="100%" stopColor={c.to} />
          </radialGradient>
        ))}
      </defs>
      {colors.map((_, i) => (
        <ellipse key={i} cx={colors[i].cx} cy={colors[i].cy} rx={colors[i].r} ry={colors[i].r} fill={`url(#hg${i})`} />
      ))}
    </svg>
  );
}

export default function App() {
  const [tab, setTab] = useState<Tab>("map");
  const [activeRoute, setActiveRoute] = useState(0);
  const [showSOS, setShowSOS] = useState(false);
  const [sheetOpen, setSheetOpen] = useState(false);
  const [alertVisible, setAlertVisible] = useState(true);
  const [searchText, setSearchText] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [mapMode, setMapMode] = useState<MapMode>("normal");
  const [showComparison, setShowComparison] = useState(false);
  const [showSearchPanel, setShowSearchPanel] = useState(false);

  const isMapTab = tab === "map";

  // Saved places for route search
  const SAVED_PLACES = [
    { icon: "🏠", label: "Home", addr: "1420 S Hyde Park Blvd" },
    { icon: "🏫", label: "College", addr: "University of Chicago" },
    { icon: "💼", label: "Workplace", addr: "123 N Michigan Ave" },
    { icon: "🏨", label: "Hostel", addr: "Oak Park Residences" },
  ];
  const RECENT = [
    { label: "42 Oak Street", time: "Yesterday" },
    { label: "Lincoln Park", time: "2 days ago" },
    { label: "River North", time: "Last week" },
  ];

  return (
    <div className="size-full flex items-center justify-center" style={{ background: "#ddd6f3" }}>
      {/* Phone shell */}
      <div
        className="relative overflow-hidden flex flex-col"
        style={{
          width: "min(390px, 100vw)",
          height: "min(844px, 100vh)",
          background: "#faf8ff",
          borderRadius: "min(44px, 4vw)",
          boxShadow: "0 40px 120px rgba(100,80,160,0.25), 0 0 0 1px rgba(124,58,237,0.12)",
        }}
      >
        {/* Status bar */}
        <div className="flex items-center justify-between px-6 pt-4 pb-1 shrink-0">
          <span className="text-foreground/70 text-xs font-medium">9:41</span>
          <div className="w-28 h-6 rounded-full bg-black absolute left-1/2 -translate-x-1/2 top-2" />
          <div className="flex items-center gap-1.5">
            <div className="flex gap-0.5 items-end">
              {[3, 5, 7, 9].map((h, i) => (
                <div key={i} className="w-1 rounded-sm bg-foreground/50" style={{ height: h }} />
              ))}
            </div>
            <div className="w-6 h-3 rounded-sm border border-foreground/40 flex items-center px-0.5">
              <div className="w-4 h-1.5 rounded-sm bg-emerald-400" />
            </div>
          </div>
        </div>

        {/* Header */}
        <div className="flex items-center justify-between px-4 pt-1 pb-3 shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #a855f7, #f43f5e)" }}>
              <Shield size={18} className="text-white" />
            </div>
            <div>
              <span className="text-foreground font-bold tracking-tight">SheMaps</span>
              <div className="flex items-center gap-1">
                <MapPin size={9} className="text-muted-foreground" />
                <span className="text-muted-foreground text-[10px]">Hyde Park, Chicago</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button className="relative w-9 h-9 rounded-xl bg-black/5 hover:bg-black/8 flex items-center justify-center transition-colors">
              <Bell size={16} className="text-foreground/70" />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-accent" />
            </button>
            <button className="w-9 h-9 rounded-xl bg-black/5 hover:bg-black/8 flex items-center justify-center transition-colors">
              <Settings size={16} className="text-foreground/70" />
            </button>
          </div>
        </div>

        {/* Alert banner */}
        <AnimatePresence>
          {alertVisible && isMapTab && (
            <AlertBanner onDismiss={() => setAlertVisible(false)} />
          )}
        </AnimatePresence>

        {/* MAP VIEW */}
        {isMapTab && (
          <div className="flex-1 relative overflow-hidden mx-4 mt-3 rounded-3xl" style={{ minHeight: 0 }}>
            <MapCanvas activeRoute={activeRoute} showDangerZones={mapMode !== "safespot"} showIncidents={mapMode === "normal"} />

            {mapMode !== "normal" && <HeatmapCanvas mode={mapMode} />}

            {/* Bottom gradient */}
            <div className="absolute bottom-0 left-0 right-0 h-24 pointer-events-none"
              style={{ background: "linear-gradient(to top, rgba(250,248,255,0.9), transparent)" }} />

            {/* Safety score */}
            <div className="absolute top-3 right-3">
              <SafetyRing score={AREA_SCORE} />
            </div>

            {/* Map mode switcher */}
            <div className="absolute top-3 left-3 flex flex-col gap-1.5">
              {([
                { mode: "normal" as MapMode, label: "Routes" },
                { mode: "heatmap" as MapMode, label: "Heatmap" },
                { mode: "safespot" as MapMode, label: "Safe Spots" },
              ]).map((m) => (
                <button
                  key={m.mode}
                  onClick={() => setMapMode(m.mode)}
                  className="flex items-center gap-1.5 px-2 py-1 rounded-lg backdrop-blur-sm transition-all text-[10px] font-medium"
                  style={{
                    background: mapMode === m.mode ? "rgba(124,58,237,0.15)" : "rgba(255,255,255,0.8)",
                    border: mapMode === m.mode ? "1px solid rgba(124,58,237,0.4)" : "1px solid rgba(0,0,0,0.1)",
                    color: mapMode === m.mode ? "#7c3aed" : "#5b21b6",
                  }}
                >
                  <Layers size={10} />
                  {m.label}
                </button>
              ))}
            </div>

            {/* Heatmap legend */}
            {mapMode === "heatmap" && (
              <div className="absolute bottom-6 left-3 right-3 flex items-center justify-center gap-3">
                {[{ color: "#10b981", label: "Safe" }, { color: "#f59e0b", label: "Moderate" }, { color: "#f43f5e", label: "Risky" }].map((l) => (
                  <div key={l.label} className="flex items-center gap-1 backdrop-blur-sm px-2 py-1 rounded-lg" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid rgba(0,0,0,0.07)" }}>
                    <div className="w-2 h-2 rounded-full" style={{ background: l.color }} />
                    <span className="text-[10px] text-foreground/80">{l.label}</span>
                  </div>
                ))}
              </div>
            )}

            {/* Route legend (normal mode) */}
            {mapMode === "normal" && (
              <>
                <div className="absolute top-14 left-3 flex flex-col gap-1.5">
                  {[
                    { color: "#10b981", label: "Safest" },
                    { color: "#f59e0b", label: "Faster" },
                    { color: "#6366f1", label: "Busy" },
                  ].map((r, i) => (
                    <button
                      key={r.label}
                      onClick={() => setActiveRoute(i)}
                      className="flex items-center gap-1.5 px-2 py-1 rounded-lg backdrop-blur-sm transition-all text-[10px] font-medium"
                      style={{
                        background: activeRoute === i ? `${r.color}20` : "rgba(255,255,255,0.75)",
                        border: activeRoute === i ? `1px solid ${r.color}55` : "1px solid rgba(0,0,0,0.08)",
                        color: activeRoute === i ? r.color : "#5b21b6",
                      }}
                    >
                      <div className="w-2 h-2 rounded-full" style={{ background: r.color }} />
                      {r.label}
                    </button>
                  ))}
                </div>

                {/* Danger zone label */}
                <div
                  className="absolute text-[9px] px-1.5 py-0.5 rounded-md text-accent font-medium backdrop-blur-sm"
                  style={{ bottom: "42%", right: "18%", background: "rgba(244,63,94,0.15)", border: "1px solid rgba(244,63,94,0.4)" }}
                >
                  ⚠ Avoid
                </div>

                {/* Compare button */}
                <button
                  onClick={() => setShowComparison(true)}
                  className="absolute bottom-6 right-3 flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-medium"
                  style={{ background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.3)", color: "#7c3aed", backdropFilter: "blur(4px)" }}
                >
                  <GitCompare size={11} /> Compare Routes
                </button>
              </>
            )}
          </div>
        )}

        {/* Non-map scrollable content */}
        {!isMapTab && (
          <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            {tab === "safety" && <SafetyIntelligence />}
            {tab === "community" && <CommunityPanel />}
            {tab === "emergency" && <EmergencyHub onSOS={() => setShowSOS(true)} />}
            {tab === "profile" && <ProfileHub />}
          </div>
        )}

        {/* Search bar (map mode) */}
        {isMapTab && (
          <div className="px-4 py-3 shrink-0">
            {showSearchPanel ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(124,58,237,0.15)", boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
                <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
                  <Search size={15} className="text-primary shrink-0" />
                  <input
                    autoFocus
                    type="text"
                    placeholder="Where are you going?"
                    value={searchText}
                    onChange={(e) => setSearchText(e.target.value)}
                    className="flex-1 bg-transparent text-foreground text-sm placeholder:text-muted-foreground outline-none"
                  />
                  <button onClick={() => setShowSearchPanel(false)}>
                    <X size={14} className="text-muted-foreground" />
                  </button>
                </div>
                {/* Saved places */}
                <div className="p-3">
                  <p className="text-[10px] text-muted-foreground font-semibold mb-2 px-1">SAVED PLACES</p>
                  {SAVED_PLACES.map((p) => (
                    <button key={p.label} onClick={() => { setSearchText(p.addr); setShowSearchPanel(false); }} className="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-muted/50 transition-colors">
                      <span className="text-base">{p.icon}</span>
                      <div className="text-left">
                        <p className="text-foreground text-sm font-medium">{p.label}</p>
                        <p className="text-muted-foreground text-xs">{p.addr}</p>
                      </div>
                    </button>
                  ))}
                  <p className="text-[10px] text-muted-foreground font-semibold mb-2 px-1 mt-3">RECENT</p>
                  {RECENT.map((r) => (
                    <button key={r.label} onClick={() => { setSearchText(r.label); setShowSearchPanel(false); }} className="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-muted/50 transition-colors">
                      <Clock size={14} className="text-muted-foreground" />
                      <div className="text-left">
                        <p className="text-foreground text-sm">{r.label}</p>
                        <p className="text-muted-foreground text-xs">{r.time}</p>
                      </div>
                    </button>
                  ))}
                </div>
              </motion.div>
            ) : (
              <button
                onClick={() => setShowSearchPanel(true)}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all"
                style={{ background: "#f0ebff", border: "1px solid rgba(124,58,237,0.15)" }}
              >
                <Search size={15} className="text-muted-foreground shrink-0" />
                <span className="flex-1 text-left text-sm text-muted-foreground">{searchText || "Where are you going?"}</span>
                <div className="w-7 h-7 rounded-lg bg-primary/20 flex items-center justify-center">
                  <Mic size={12} className="text-primary" />
                </div>
              </button>
            )}
          </div>
        )}

        {/* Bottom sheet for routes (map) */}
        {isMapTab && mapMode === "normal" && (
          <div className="shrink-0 mx-4 rounded-t-2xl overflow-hidden transition-all"
            style={{ background: "rgba(255,255,255,0.97)", border: "1px solid rgba(124,58,237,0.12)", borderBottom: "none" }}
          >
            <button onClick={() => setSheetOpen((p) => !p)} className="w-full flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-2">
                <Navigation size={14} className="text-primary" />
                <span className="text-foreground text-sm font-medium">Route Options</span>
                <span className="text-muted-foreground text-xs">· 3 safe routes found</span>
              </div>
              {sheetOpen ? <ChevronDown size={14} className="text-muted-foreground" /> : <ChevronUp size={14} className="text-muted-foreground" />}
            </button>
            <AnimatePresence>
              {sheetOpen && (
                <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} style={{ overflow: "hidden" }}>
                  <RoutesPanel activeRoute={activeRoute} onSelectRoute={(i) => { setActiveRoute(i); setSheetOpen(false); }} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        )}

        {/* Bottom nav */}
        <div className="shrink-0 px-2 pb-6 pt-2 flex items-center justify-around"
          style={{ background: "#faf8ff", borderTop: "1px solid rgba(124,58,237,0.1)" }}
        >
          {TABS.map((t) => {
            const Icon = t.icon;
            const active = tab === t.id;
            const isEmergency = t.id === "emergency";
            if (isEmergency) {
              return (
                <button
                  key={t.id}
                  onClick={() => setTab(t.id)}
                  className="relative flex flex-col items-center justify-center -mt-4"
                >
                  <div
                    className="w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all"
                    style={{
                      background: active ? "linear-gradient(135deg, #f43f5e, #be123c)" : "linear-gradient(135deg, #f43f5e, #be123c)",
                      boxShadow: "0 4px 20px rgba(244,63,94,0.45)",
                      transform: active ? "scale(1.08)" : "scale(1)",
                    }}
                  >
                    <Siren size={22} className="text-white" />
                  </div>
                  <span className="text-[10px] mt-1" style={{ color: active ? "#f43f5e" : "#7c6a9e" }}>Emergency</span>
                </button>
              );
            }
            return (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className="flex flex-col items-center gap-1 px-2 py-1 rounded-xl transition-all"
                style={{ color: active ? "#7c3aed" : "#9d8fc2" }}
              >
                <Icon size={20} fill={active ? "currentColor" : "none"} strokeWidth={active ? 2 : 1.5} />
                <span className="text-[10px] font-medium">{t.label}</span>
              </button>
            );
          })}
        </div>
      </div>

      {/* Route Comparison Modal */}
      <AnimatePresence>
        {showComparison && (
          <RouteComparison
            onClose={() => setShowComparison(false)}
            onSelect={(i) => { setActiveRoute(i); setShowComparison(false); }}
            activeRoute={activeRoute}
          />
        )}
      </AnimatePresence>

      {/* SOS Modal */}
      <AnimatePresence>
        {showSOS && <SOSModal onClose={() => setShowSOS(false)} />}
      </AnimatePresence>
    </div>
  );
}

// Missing import for Clock icon used in search panel
function Clock({ size, className }: { size: number; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={className}>
      <circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" />
    </svg>
  );
}
