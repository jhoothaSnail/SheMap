...
export function MapCanvas({ activeRoute = 0, showDangerZones = true, showIncidents = true }: MapCanvasProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      bg.addColorStop(0.5, "#f8dfe9");
      bg.addColorStop(1, "#fff8fb");
      ctx.fillStyle = bg;
      ctx.fillRect(0, 0, W, H);

      // Grid lines (city blocks)
      const gridColor = "rgba(201, 64, 118, 0.13)";
      ...
      // Major roads
      ctx.strokeStyle = "rgba(95, 49, 74, 0.18)";
      ctx.lineWidth = 2.5;
      ...
      if (showIncidents) {
        ...
        incidents.forEach((inc) => {
          const px = inc.x * W;
          const py = inc.y * H;
          ctx.fillStyle = inc.type === "alert" ? "rgba(244, 63, 94, 0.9)" : "rgba(201, 64, 118, 0.9)";
          ctx.shadowBlur = 8;
          ctx.shadowColor = inc.type === "alert" ? "#f43f5e" : "#c94076";
          ctx.beginPath();
      ...
      locGrd.addColorStop(0, "rgba(201, 64, 118, 0.35)");
      locGrd.addColorStop(1, "rgba(201, 64, 118, 0)");
      ctx.fillStyle = locGrd;
      ctx.beginPath();
      ctx.arc(locX, locY, 28, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = "#c94076";
      ctx.shadowBlur = 12;
      ctx.shadowColor = "#c94076";
      ctx.beginPath();
